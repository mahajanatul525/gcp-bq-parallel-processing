...
def process_batch(batch_num, records):
    ...
    json_data = json.dumps(records, indent=2, default=custom_serializer)
...
# Function to fetch data from BigQuery in parallel batches using ProcessPool
def fetch_data_in_batches():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for batch_num, page in enumerate(iterator.pages):
            records = [dict(row) for row in page]  # Ensure serializable data
